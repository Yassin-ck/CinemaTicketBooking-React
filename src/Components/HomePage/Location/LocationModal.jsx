...
{locationFetched &&
  locationFetched.map(item => (
    ...
    <input
        ...
        {
          (newLocationRequestData && (newLocationRequestData.country !== '' && newLocationRequestData.state !== '' && newLocationRequestData.district !== '' && newLocationRequestData.place !== '')) ? (
             <div className='AddNewLocationRequestButton' onClick={VerifyingNewLocationReqeust}>
            <MdOutlineDone size={20}  style={{ color: 'green' }} />
            </div>
          ) : (!locationFetched ? (
            <div className='AddNewLocationRequestButton' onClick={newLocationRequestHandler}>
              {user?<IoAddSharp size={20} style={{ color: 'green' }} />:<IoAddSharp  size={20} style={{ color: 'grey',cursor:'progress' }} />}
            </div>
          ) : null)
        }
          ...
          {(locationFetched === undefined || (locationFetched !== undefined && locationFetched.length === 0)) && (
            ...
            <MDBModalBody className="ModalContentOfTheLocation">{newLocationRequest&&<>
              ...
              <div className=' inpurSeparatingDivForNewLocationRequest col-md-12'>
              <div className="newLocationRequestInputClassName col-md-6">
              <input type='text' name="district" required autoComplete='off' onChange={e=>locationValueCollecter(e)} />
              <label for="district">district</label>
              </div>
              <div className="newLocationRequestInputClassName col-md-6">
              <input  type='text' name="place" required autoComplete='off' onChange={e=>locationValueCollecter(e)}  />
